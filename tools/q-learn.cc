...
template <class S>
int QLearn<S>::get_action(S state) {
    ++num_iters_;
...
void QLearn<S>::incorporate_feedback(S state, int action,
    ...
    std::map<std::string, double> features = featurize(state, action);
...
template <class S>
double QLearn<S>::get_q(S state, int action) const {
    std::map<std::string, double> features = featurize(state, action);
    double score = 0;
    for (const auto& kv : features) {
...
template <class S>
double QLearn<S>::get_v_opt(S state) const {
