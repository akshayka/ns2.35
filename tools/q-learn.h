...
public:
    ...
    virtual int get_action(S state) const;
...
private:
    ...
    std::map<string, double> weights_;

    // TODO(akshayka): Define a custom key class
    // map state-action pairs to weights
    std::map<std::pair<S, int>, std::vector<double>> weights_;
